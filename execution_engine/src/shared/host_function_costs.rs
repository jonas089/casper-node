...
pub struct HostFunctionCosts {
    ...
    pub circom_verifier: HostFunction<[Cost; 6]>,
}
...
impl Default for HostFunctionCosts {
    fn default() -> Self {
        Self {
            ...
            circom_verifier: HostFunction::default()
        }
    }
}
...
impl ToBytes for HostFunctionCosts {
    fn to_bytes(&self) -> Result<Vec<u8>, bytesrepr::Error> {
        ...
        ret.append(&mut self.circom_verifier.to_bytes()?);
        Ok(ret)
    }
    ...
    fn serialized_length(&self) -> usize {
        self.read_value.serialized_length()
            ...
            + self.circom_verifier.serialized_length()
    }
}
...
impl FromBytes for HostFunctionCosts {
    fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), bytesrepr::Error> {
        ...
        let (circom_verifier, rem) = FromBytes::from_bytes(rem)?;
        Ok((
            HostFunctionCosts {
                read_value,
                dictionary_get,
...
pub mod gens {
    ...
    prop_compose! {
        pub fn host_function_costs_arb() (
            ...
            circom_verifier in host_function_cost_arb()
        ) -> HostFunctionCosts {
            HostFunctionCosts {
                read_value,
                dictionary_get,
