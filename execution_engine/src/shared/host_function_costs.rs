...
pub struct HostFunctionCosts {
    ...
    // miden verifier host function
    pub miden_verifier: HostFunction<[Cost; 2]>,
}
...
impl Default for HostFunctionCosts {
    fn default() -> Self {
        Self {
            ...
            miden_verifier: HostFunction::default(),
        }
    }
}
...
impl ToBytes for HostFunctionCosts {
    fn to_bytes(&self) -> Result<Vec<u8>, bytesrepr::Error> {
        ...
        ret.append(&mut self.miden_verifier.to_bytes()?);
        Ok(ret)
    }
    ...
    fn serialized_length(&self) -> usize {
        self.read_value.serialized_length()
            ...
            + self.miden_verifier.serialized_length()
    }
}
...
impl FromBytes for HostFunctionCosts {
    fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), bytesrepr::Error> {
        ...
        let (miden_verifier, rem) = FromBytes::from_bytes(rem)?;
        Ok((
            HostFunctionCosts {
                read_value,
                dictionary_get,
...
pub mod gens {
    ...
    prop_compose! {
        pub fn host_function_costs_arb() (
            ...
            miden_verifier in host_function_cost_arb()
        ) -> HostFunctionCosts {
            HostFunctionCosts {
                read_value,
                dictionary_get,
